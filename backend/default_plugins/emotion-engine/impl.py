...
class EmotionEngineStore:
    ...
    def _load_state(self) -> EmotionState:
        ...
        state.history = [item for item in list(raw.get("history") or [])[-HISTORY_LIMIT:] if isinstance(item, dict)]
        ...
        state.dominant_emotion = str(raw.get("dominant_emotion") or self._dominant_from_vector(state.vector))
        return state
    ...
    def _history_24h(self) -> list[dict[str, Any]]:
        cutoff = _now() - 86400
        return [item for item in self._state.history if float(item.get("ts") or 0) >= cutoff]

    def _record_history(self, reason: str, deltas: dict[str, float] | None = None) -> None:
    ...
    def _top_emotions(self, count: int = 3) -> list[tuple[str, float]]:
        return sorted(((key, float(self._state.vector.get(key, 0.0))) for key in EMOTION_KEYS), key=lambda item: item[1], reverse=True)[:count]
    ...
    def snapshot(self, config: dict[str, Any] | None = None) -> dict[str, Any]:
        with self._lock:
            ...
            return {
                ...
                "pending_corememory_sync": self._state.pending_corememory_sync,
                "dominant_emotion": self._state.dominant_emotion,
                "top_emotions": [{"key": key, "label": EMOTION_LABELS[key], "value": value} for key, value in self._top_emotions()],
                "updated_at": int(self._state.last_interaction_ts),
                "config": {
                    "sharp_tongue_enabled": bool(cfg.get("SHARP_TONGUE_REWRITE", False)),
    ...
    def apply_emotion_tag(self, tag: str) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            for key, delta in deltas.items():
                self._state.vector[key] = _clamp(self._state.vector.get(key, 0.0) + delta)
            if self._state.recent_user_modes[-3:] == ["task", "task", "task"]:
                self._state.vector["sharpness"] = _clamp(self._state.vector.get("sharpness", 0.0) + 1.0)
            ...
            return {
                "applied": True,
                "tag": emotion_key,
                "significant": any(abs(delta) >= 1.5 for delta in deltas.values()) or before != after,
    ...
    def heartbeat(self, decay_per_minute: float) -> dict[str, Any]:
        with self._lock:
            ...
            for key in EMOTION_KEYS:
                if key == "boredom":
                    continue
                self._state.vector[key] = _clamp(self._state.vector.get(key, 0.0) - decay_per_minute * decay_units)
            self._state.last_decay_ts = _now()
            self._state.silence_ticks += 1
            boredom_bump = False
            if self._state.silence_ticks >= 6:
                self._state.vector["boredom"] = _clamp(self._state.vector.get("boredom", 0.0) + 1.0)
                self._state.silence_ticks = 0
                boredom_bump = True
    ...
    def build_prompt_suffix(self, config: dict[str, Any]) -> str:
        with self._lock:
            ...
            lines = [
                ...
                "当前向量: " + "; ".join(f"{EMOTION_LABELS[key]}={value:.1f}" for key, value in top),
            ]
            return "\n\n[emotion_engine]\n" + "\n".join(lines)

    def should_write_diary(self, reason: str | None, significant: bool) -> bool:
        with self._lock:
            return bool(reason and significant and (_now() - self._state.last_diary_ts) >= 900)
...
class Plugin(FaustPlugin):
    ...
    def startup(self, ctx: PluginContext) -> None:
        ...
        ctx.register_config([
            {"key": "SHARP_TONGUE_REWRITE", "type": "bool", "label": "毒舌改写开关", "default": False},
            {"key": "DECAY_PER_MINUTE", "type": "float", "label": "情绪衰减速率", "default": 0.1},
            {"key": "OVERLAY_INTENSITY", "type": "int", "label": "滤镜强度", "default": 50},
        ])
        ...
        ctx.vfs_write_symbolic(
            ...
            lambda _path: json.dumps(self.get_state_payload(), ensure_ascii=False, indent=2),
        )
    ...
    def register_frontend(self) -> list[dict]:
        return [
            {"type": "js", "path": "/faust/plugins/emotion-engine/frontend/panel-v2.js"},
            {"type": "js", "path": "/faust/plugins/emotion-engine/frontend/app-hook-v2.js"},
            {"type": "css", "path": "/faust/plugins/emotion-engine/frontend/panel-v2.css"},
        ]

    def register_prompt_suffix(self) -> list[str]:
        return [self.store.build_prompt_suffix(self._configs()) if self.store is not None else ""]
    ...
    def message_sent(self, msg: str, response: Any, ctx: PluginContext) -> Any:
        ...
        if analysis and self.store.should_write_diary(str(analysis.get("diary_reason") or ""), bool(analysis.get("significant"))):
    ...
    def heartbeat(self, ctx: PluginContext) -> None:
        if self.store is None:
            return
        decay = float(self.ctx.get_config("DECAY_PER_MINUTE", 0.1) or 0.1) if self.ctx else 0.1
        result = self.store.heartbeat(decay)
        if result.get("boredom_bump") and self.store.should_write_diary("长时间沉默导致无聊上升", True):
